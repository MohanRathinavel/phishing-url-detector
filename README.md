...
>>>>>>> aeebcae097c90a0e3f48598242e592ce63295e4a
phishing-detector/
│
├── model/
...
### `GET /health`
```json
{ "status": "ok", "model": "Random Forest", "model_ready": true }
```

### `GET /plots`
```json
{ "plots": ["cm_random_forest.png", "feature_importance.png", ...] }
```
...
| URL | Expected Result |
|-----|----------------|
| `https://github.com/openai/whisper` | ✅ Safe (~97%) |
| `https://stackoverflow.com/questions/python` | ✅ Safe (~95%) |
| `http://192.168.1.1/login/verify@secure` | 🚨 Phishing (~99%) |
| `http://paypal-secure-update.xyz/account/login.php` | 🚨 Phishing (~98%) |
| `http://signin-abc123.tk/update?ref=xyz` | 🚨 Phishing (~97%) |
